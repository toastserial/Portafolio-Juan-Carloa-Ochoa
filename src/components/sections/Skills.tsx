    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setIsVisible(true); // ✅ Activa animación al entrar
        } else {
          setIsVisible(false); // 🔄 Reinicia cuando sale de la pantalla
        }
      },
      { threshold: 0.2 } // 20% visible para activar/desactivar
    );
  ...
  return (
    ...
    >
      <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        {/* Skills Grid */}
        <div
          className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-16 transition-all duration-1000 delay-400 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
        >
        ...
        >
          ...
          <div className="p-6 text-center transition-transform duration-300 bg-gradient-to-br from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 rounded-xl hover:scale-105">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Experiencia con metodologías ágiles y mejores prácticas de
              desarrollo
            </p>
          </div>
        ...
        >
          ...
          <p className="mb-6 text-gray-600 dark:text-gray-400">
            Estoy disponible para colaborar en proyectos desafiantes y
            emocionantes
          </p>
